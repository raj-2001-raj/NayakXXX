...
-- RLS for spatial_ref_sys (read-only, allow all)
ALTER TABLE public.spatial_ref_sys ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read spatial_ref_sys" ON public.spatial_ref_sys
  FOR SELECT TO anon, authenticated
  USING (true);

-- RLS for fountains (read-only, allow all)
ALTER TABLE public.fountains ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read fountains" ON public.fountains
  FOR SELECT TO anon, authenticated
  USING (true);

-- RLS for accident_stats (read-only, allow all)
ALTER TABLE public.accident_stats ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read accident_stats" ON public.accident_stats
  FOR SELECT TO anon, authenticated
  USING (true);

-- RLS for surface_segments (read-only, allow all)
ALTER TABLE public.surface_segments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read surface_segments" ON public.surface_segments
  FOR SELECT TO anon, authenticated
  USING (true);

-- Set search_path for flagged functions (run as superuser if needed)
ALTER FUNCTION public.submit_ride_batch() SET search_path = public;
ALTER FUNCTION public.update_path_score_from_anomaly() SET search_path = public;
ALTER FUNCTION public.calculate_path_score() SET search_path = public;
ALTER FUNCTION public.get_anomalies_in_bbox(double precision, double precision, double precision, double precision, integer) SET search_path = public;
ALTER FUNCTION public.deduplicate_anomalies() SET search_path = public;
ALTER FUNCTION public.check_duplicate_anomaly() SET search_path = public;

-- NOTE: Moving extensions requires superuser. If possible, move postgis to a separate schema, e.g.:
-- CREATE SCHEMA extensions;
-- ALTER EXTENSION postgis SET SCHEMA extensions;

-- NOTE: Materialized view public.anomaly_hotspots is selectable by anon/authenticated. Restrict if needed.

-- NOTE: Enable leaked password protection in Supabase Auth settings UI.
